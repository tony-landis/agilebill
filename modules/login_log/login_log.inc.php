...
class login_log
{
	...
	function graph($start, $end, $constraint, $default)
	{
		...
		$db     = &DB();
		$sql    = 'SELECT date_orig FROM ' . AGILE_DB_PREFIX . 'login_log WHERE
		...
		$C_translate->value['login_log']['count'] = $result->RecordCount();
		$title = $C_translate->translate('statistics_login','login_log','');
		$arr['title']   = $title;
		$arr['results'] = $default;
		return $arr;
	}
