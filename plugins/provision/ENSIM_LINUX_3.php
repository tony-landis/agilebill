...
if(empty($VAR))
{
    ...
    $plg = new plgn_prov_ENSIM_LINUX_3();
    $plg->p_all($VAR);
}


# Main Class
class plgn_prov_ENSIM_LINUX_3
{
    function plgn_prov_ENSIM_LINUX_3()
    {
        $this->name             = 'ENSIM_LINUX_3';
    ...
    function p_new()
    {
        ...
        if ($this->plugin_data['ipinfo_namebased'] == '1' && $this->ip != false)
        {
            $cmd =  "AddVirtDomain "        .
                    ...
                    " -c siteinfo,domain="  . $this->service['domain_name']. "." . $this->service['domain_tld'] .
                    ",admin_user=\""        . $this->login['username'] . "\"".
                    ",tpasswd="             . $this->login['password'] .
                    ",email="               . $this->account['email'] .
                    " -c ipinfo,namebased=0,ipaddrs=\[" . $this->ip . "\]" . $this->nl;
        }
        else
        {
            $cmd =  "AddVirtDomain "        .
    ...
    function p_edit()
    {
        $cmd =  "EditVirtDomain -p " . $this->plugin_data['plan'] .
                " -c siteinfo,".
                "email=" . $this->account['email'] .
                ",domain=" . $this->service['domain_name']. "." . $this->service['domain_tld'] .
                " -D " . $this->service['domain_name']. "." . $this->service['domain_tld'];
        return $cmd;
    }
