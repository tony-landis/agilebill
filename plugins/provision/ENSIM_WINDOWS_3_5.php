...
if(empty($VAR))
{
    ...
    $plg = new plgn_prov_ENSIM_WINDOWS_3_5();
    $plg->p_all($VAR);
}


# Main Class
class plgn_prov_ENSIM_WINDOWS_3_5
{
    function plgn_prov_ENSIM_WINDOWS_3_5()
    {
        $this->name             = 'ENSIM_WINDOWS_3_5';
    ...
    function p_new()
    {
        ...
        '<sendEmail>0</sendEmail><from>na@na.com</from><emailContent>hello</emailContent><subject>hello</subject>'.
        '<reseller></reseller>'.
        '<setDns>1</setDns>'.
        '<dataDrive>'           . $this->plugin_data['dataDrive']   . '</dataDrive>'.
        '<style_name>'          . $this->plugin_data['style_name']  . '</style_name>'.
        ...
        '<winanalog>'           . $this->plugin_data['winanalog']   . '</winanalog>' .
        ...
        '<urchin>'              . $this->plugin_data['urchin']      . '</urchin>' .
        '<mssqlodbc>'           . $this->plugin_data['odbc']        . '</mssqlodbc>' .
        '<msftpsvc>'.
        ...
        '<maxclientsmessage>TO_MANY_USERS</maxclientsmessage>'.
        ...
        '<accesssslflags>0</accesssslflags>' .
        '<execflag>1</execflag>' .
        ...
        '<enablecpuquota>'          . $this->plugin_data['enablecpuquota']          . '</enablecpuquota>' .
    ...
    function p_edit()
    {
        # defaults
        while (list($key, $val) = each(@$this->plugin_data)) {
            if (empty($this->plugin_data[$key]) || $this->plugin_data[$key] == false)
                $this->plugin_data[$key] = '0';
        }
        ...
        '<xml>'.
        '<domain>'              . $this->service['domain_name']. '.' . $this->service['domain_tld'] .     '</domain>'.
        '<user>'                . $this->service['host_username']          . '</user>'.
        '<password>'            . $this->service['host_password']          . '</password>'.
        '<email>'               . $this->account['email']           . '</email>'.
        ...
        '<exitmessage>Goodbye</exitmessage>' .
        '<greetingmessage>Hello</greetingmessage>' .
        '<maxclientsmessage>TryLater</maxclientsmessage>' .
        '<msftpsvc>'                . $this->plugin_data['msftpsvc'] . '</msftpsvc>' .
        '</msftpsvc>'.
        '<w3svc>' .
        '<cgi>'                     . $this->plugin_data['cgi']         . '</cgi>' .
        '<ssi>'                     . $this->plugin_data['ssi']         . '</ssi>' .
        '<sslc>'                    . $this->plugin_data['sslc']        . '</sslc>' .
        '<maxconnectionsunlimited>' . $this->plugin_data['w3svc_maxconnectionsunlimited'] . '</maxconnectionsunlimited>' .
        '<allowkeepalive>1</allowkeepalive>' .
        '<appisolated>2</appisolated>' .
        '<enablereversedns>1</enablereversedns>' .
    ...
    function p_inactive()
    {
        return 'editdomain -s "<xml><domain>'.$this->service['domain_name'].'.'.$this->service['domain_tld'].'</domain><services><w3svc><w3svc>0</w3svc></w3svc><winmail><winmail>0</winmail></winmail><msftpsvc><msftpsvc>0</msftpsvc></msftpsvc></services></xml>"';
    }

    # deactivate service
    function p_active()
    {
        return 'editdomain -s "<xml><domain>'.$this->service['domain_name'].'.'.$this->service['domain_tld'].'</domain><services><w3svc><w3svc>'.$this->plugin_data['w3svc'].'</w3svc></w3svc><winmail><winmail>'.$this->plugin_data['winmail'].'</winmail></winmail><msftpsvc><msftpsvc>'.$this->plugin_data['msftpsvc'].'</msftpsvc></msftpsvc></services></xml>"';
    }
    ...
    function p_delete()
    {
        ...
        return 'deldomain -s "<xml><domain>'.$this->service['domain_name']. "." . $this->service['domain_tld'].'</domain></xml>"';
    }
