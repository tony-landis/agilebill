...
{$method->exe("file_category","search_show")}
      ...
      <td>
        <table id="main2" width="100%" border="0" cellspacing="1" cellpadding="2">
          ...
          <tr valign="middle" align="center" class="table_heading">
              ...
              <td width="893" class="table_heading"> 
                {literal}
                <script language="JavaScript">
					document.write(search_heading('{/literal}{translate module=file_category}field_name{/translate}{literal}','name'));
				 </script>
              {/literal}
            </td>
              <td width="46" class="table_heading">&nbsp; </td>
			 <!-- LOOP THROUGH EACH RECORD -->
			 {foreach from=$file_category item=record}
             <tr id="row{$record.id}" onClick="row_sel('{$record.id}',1);" onDblClick="window.location='?_page=file_category:view&id={$record.id},{$COOKIE_URL}';" onMouseOver="row_mouseover('{$record.id}', 'row_mouse_over_select', 'row_mouse_over');" onMouseOut="row_mouseout('{$record.id}', '{$record._C}', 'row_select');" class="{$record._C}">
